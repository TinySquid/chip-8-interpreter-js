...
export default class Chip8 {
	...
	constructor(renderer, keypad, speaker, options) {
		...
		this.chunk = {
			...
			cyclesPerInterval: this.options["cyclesPerSecond"] / (1000 / this.options["chunkIntervalMs"]),
		};
	...
	start() {
		...
		this.chunk.handler = setInterval(this.runChunk.bind(this), this.chunk.intervalMs);
    ...
    */
		...
		// FETCH
		const instruction = (this.memory[this.registers["PC"]] << 8) | this.memory[this.registers["PC"] + 1];
    ...
    */
		const opcode = [(instruction & 0xf000) >> 12, instruction & 0x00ff, instruction & 0x000f];
		...
		switch (opcode[0]) {
			...
			// 5XY0
			case 0x5:
				if (this.registers[vX] === this.registers[vY]) this.registers["PC"] += 2;
				break;
			...
			case 0xf:
				switch (opcode[1]) {
					...
					// FX29
					case 0x29:
						this.registers["I"] = this.options.defaultFontStartAddress + this.registers[vX] * 5;
						break;
					...
					case 0x33:
						...
						this.memory[this.registers["I"] + 1] = Math.floor((this.registers[vX] % 100) / 10);
    ...
    */
	...
	printRegisters() {
		let prettyString = `PC: ${this.registers["PC"]} (0x${this.registers["PC"].toString(
			16
		)}) | I: ${this.registers["I"]} (0x${this.registers["I"].toString(16)})\n`;
